---
name: planner-status-report
description: Generates a Microsoft Planner status report for one plan, using task status, owners, dates, priorities, buckets, risks, wins, progress, upcoming commitments, and milestones.
---
...
- Multi-plan rollups or comparisons. This skill is scoped to one plan.
- General "how is the project?" or "catch me up" requests without an explicit report/update/summary/briefing noun.

## Instructions

> **Pattern: Fetch + Local Analysis, with Ask as fallback.** Use `fetch_work_iq` with deterministic Microsoft Graph-style paths for plan discovery and structured JSON. Use `ask_work_iq` only for semantic context or after the canonical structured plan-resolution paths are exhausted, unavailable, or policy-blocked. Do the report specification extraction, task categorization, statistics, and final composition locally in the skill. Do not invent data.
...
If the plan ID is already available from context, use it directly. Otherwise follow the canonical named-plan resolution workflow in [Tasks (Planner)](../../../workiq/skills/workiq/references/tasks-work-iq.md#find-the-plan-before-using-ask-required-for-named-plan-requests). That shared guidance is authoritative for lookup order, paging, group-backed plans, filtering requirements, and semantic fallback behavior.
...
When using `$select`, limit it to the supported `plannerTask` properties listed above;
